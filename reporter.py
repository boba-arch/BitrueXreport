...
def generate_report() -> tuple[str, str]:
    ...
    Pulls every tweet that has ALREADY been classified (by the scrape+classify
    +alert cycle) since the last report -- both useful (already alerted) and
    not-useful ones -- so the report is a full record, not just the alerts.
    """
